    ...
    Tick newTime -> (
        {
          ...
          |> List.map (\x -> case x of 
            CustomClock clock -> CustomClock (updateClock clock newTime)
            Section s -> Section s)
        }
      , Cmd.none
      )
...
displayClock clock =
  let
    hour   = String.fromInt (Time.toHour   clock.zone clock.time)
    minute = String.fromInt (Time.toMinute clock.zone clock.time)
    second = String.fromInt (Time.toSecond clock.zone clock.time)
  in
  div [] [ 
    text clock.title
